...
int main(void){
    ...
    shared_struct array[100000];        /*guarda o que ler da memória partilhada*/
    shared_struct *pt;
    ...
    fd = shm_open("/ex5",O_RDWR,S_IRUSR|S_IWUSR);
    ...
    /*imprimir as 10 primeiras posições do array*/
    for(i = 0; i < 9; i++){
        printf("st:     %s\n", array[i].st);
        printf("num     %d\n", array[i].num);
    }
